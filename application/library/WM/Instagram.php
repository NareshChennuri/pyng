...
class WM_Instagram {
    ...
    protected $_endpointUrls = array(
    	...
    	'user_media' => 'https://api.instagram.com/v1/users/%s/media/recent/?access_token=%s&count=%s',
    ...
    */
     ...
     * @param WM_Instagram_Curlhttpclient $httpClient
     */
    ...
    public function __construct($config = null, $arrayResponses = false) {
        ...
        if (empty($config)) {
            throw new WM_Instagram_Exception('Configuration params are empty or not an array.');
        }
    }
     ...
     * @param string $method
     */
    protected function _initHttpClient($uri, $method = WM_Instagram_Curlhttpclient::GET) {
        if ($this->_httpClient == null) {
            $this->_httpClient = new WM_Instagram_Curlhttpclient($uri);
        } else {
    ...
    protected function _setOauthToken() {
        $this->_initHttpClient($this->_endpointUrls['access_token'], WM_Instagram_Curlhttpclient::POST);
    ...
    public function getAccessToken() {  
        if ($this->_accessToken == null) {
			...
			$_oauthToken = json_decode($this->_oauthToken);
			if(isset($_oauthToken->access_token)) {
    ...
    public function getCurrentUser() {
        if ($this->_currentUser == null) {
			...
			$_currentUser = json_decode($this->_oauthToken);
			if(isset($_currentUser->user)) {
            	$this->_currentUser = $_currentUser->user;
			}
        }
    ...
    public function getUserMedia($id = 'self', $limit = 0) {
        $endpointUrl = sprintf($this->_endpointUrls['user_media'], $id, $this->getAccessToken(), $limit);
        $this->_initHttpClient($endpointUrl);
        return $this->_getHttpClientResponse();
    }
    ...
    public function modifyUserRelationship($id, $action) {
        ...
        $this->_initHttpClient($endpointUrl, WM_Instagram_Curlhttpclient::POST);
    ...
    public function deleteComment($mediaId, $commentId) {
        ...
        $this->_initHttpClient($endpointUrl, WM_Instagram_Curlhttpclient::DELETE);
        return $this->_getHttpClientResponse();
    }

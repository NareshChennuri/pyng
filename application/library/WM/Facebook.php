...
//     CURLOPT_USERAGENT      => 'facebook-php-2.0',
  ...
  /**
   * Indicates if Session support should be enabled.
   */
  protected $sessionSupport = false;
  ...
  public function __construct($config) {
    ...
    if (isset($config['session'])) {
      $this->setSessionSupport($config['session']);
    }
    ...
    $this->setNextUrl($this->getCurrentUrl());
  }
  
	/**
   * Set the next step url.
   *
   * @param String $url the next step url
   */
  ...
  /**
   * Set the Session Support status.
   *
   * @param Boolean $sessionSupport the Session Support status
   */
  public function setSessionSupport($sessionSupport) {
  ...
  /**
   * Get the Session Support status.
   *
   * @return Boolean the Session Support status
   */
  public function useSessionSupport() {
  ...
  public function setSession($session=null, $write_cookie=true) {
    ...
    if ($write_cookie) {
      ...
      $this->setSessionFromSession($session);
    }
    return $this;
  }
...
//      if(!$session && $this->useSessionSupport()) {
//      	$cookieName = $this->getSessionCookieName();
//      	if(JO_Session::get($cookieName)) {
//      		$session = JO_Registry::get($cookieName);
...
//      }
//      
//      if(!$session && JO_Session::get('user[facebook_session]')) {
...
//	  }
  ...
  protected function _restserver($params) {
    ...
    if (is_array($result) && isset($result['error_code'])) {
    	return false;
//    	by joro
//      throw new WM_Facebook_Exception($result);
    }
    return $result;
  }
...
//      $e = new WM_Facebook_Exception($result);
//      switch ($e->getType()) {
...
//		by joro
  ...
  protected function makeRequest($url, $params, $ch=null) {
	...
	if(!$result) {
		...
		if(isset($params['method']) && strtolower($params['method']) == 'post') {
			$send['http']['method'] = 'POST';
			$send['http']['header'] = "Content-type: application/x-www-form-urlencoded\r\n"
			. "Content-Length: " . strlen($content) . "\r\n";
		} else {
			$send['http']['method'] = 'GET';
			$url .= ( strpos($url, '?') ? '&' : '?' ) . $content;
		}
		$send['http']['content'] = $content;
		$context  = stream_context_create($send); 
		$result = @file_get_contents($url, false, $context);
	}

    if ($result === false) {
//      $e = new WM_Facebook_Exception(array(
...
//		by joro
  ...
  protected function setCookieFromSession($session=null) {
    ...
    // @codeCoverageIgnoreStart
      ...
      $this->createCookie($cookieName, $value, $expires, '/', $domain);
    }
  ...
  public function deleteSession() {
	  ...
	  if (headers_sent()) {
	    ...
	    // @codeCoverageIgnoreStart
	      ...
	      setcookie($cookieName, null, time(), '/', $this->getBaseDomain());
	      $this->createCookie($cookieName, null, time(), '/', $this->getBaseDomain());
	    }
  }
   ...
   * @param Array $session the session to use for setting the cookie
   */
  protected function setSessionFromSession($session=null) {
    if (!$this->useSessionSupport()) {
      return;
    }

    $cookieName = $this->getSessionCookieName();
    
    if(!$session && !JO_Session::get($cookieName)) {
    	return;
    }
    
    JO_Session::set($cookieName, $session);
  ...
  protected function validateSessionObject($session) { 
    ...
    /*if (is_array($session) &&
      ...
      $expected_sig = self::generateSignature(
        $session_without_sig,
        $this->getApiSecret()
      ); var_dump($session['sig'], $expected_sig); exit;

...
class JO_Mailer_Base {
	...
	public $attachmendImagesBody = true;
	...
	protected function AddAnAddress($kind, $address, $name = '') {
		if (!preg_match('/^(to|cc|bcc|Reply-To)$/', $kind)) {
			...
			if ($this->exceptions) {
				throw new JO_Mailer_Exception('Invalid recipient array: ' . $kind);
			}
			if ($this->SMTPDebug) {
		...
		if (!self::ValidateAddress($address)) {
			...
			if ($this->exceptions) {
				throw new JO_Mailer_Exception($this->Lang('invalid_address').': '.$address);
			}
			if ($this->SMTPDebug) {
	...
	protected function PreSend() {
		try {
			...
			if ((count($this->to) + count($this->cc) + count($this->bcc)) < 1) {
				throw new JO_Mailer_Exception($this->Lang('provide_address'), self::STOP_CRITICAL);
			}
			...
			if (empty($this->Body)) {
				//throw new JO_Mailer_Exception($this->Lang('empty_message'), self::STOP_CRITICAL);
			}
		...
		if ($this->SingleTo === true) {
			foreach ($this->SingleToArray as $key => $val) {
				if(!@$mail = popen($sendmail, 'w')) {
					throw new JO_Mailer_Exception($this->Lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
				}
				fputs($mail, "To: " . $val . "\n");
				fputs($mail, $header);
				fputs($mail, $body);
	...
	protected function MailSend($header, $body) {
		...
		if(!$rt) {
			throw new JO_Mailer_Exception($this->Lang('instantiate'), self::STOP_CRITICAL);
		}
		return true;
	}
	...
	protected function SmtpSend($header, $body) {
		...
		if(!$this->SmtpConnect()) {
			throw new JO_Mailer_Exception($this->Lang('smtp_connect_failed'), self::STOP_CRITICAL);
		}
		$smtp_from = ($this->Sender == '') ? $this->From : $this->Sender;
		if(!$this->smtp->Mail($smtp_from)) {
			throw new JO_Mailer_Exception($this->Lang('from_failed') . $smtp_from, self::STOP_CRITICAL);
		}
		...
		if (count($bad_rcpt) > 0 ) { //Create error message for any bad addresses
			...
			throw new JO_Mailer_Exception($this->Lang('recipients_failed') . $badaddresses);
		}
		if(!$this->smtp->Data($header . $body)) {
			throw new JO_Mailer_Exception($this->Lang('data_not_accepted'), self::STOP_CRITICAL);
		}
				...
				if ($this->smtp->Connect(($ssl ? 'ssl://':'').$host, $port, $this->Timeout)) {
					...
					if ($tls) {
						if (!$this->smtp->StartTLS()) {
							throw new JO_Mailer_Exception($this->Lang('tls'));
						}
					...
					if ($this->SMTPAuth) {
						if (!$this->smtp->Authenticate($this->Username, $this->Password)) {
							throw new JO_Mailer_Exception($this->Lang('authenticate'));
						}
					}
				}
				$index++;
				if (!$connection) {
					throw new JO_Mailer_Exception($this->Lang('connect_host'));
				}
			}
		} catch (JO_Mailer_Exception $e) {
			$this->smtp->Reset();
		...
		if ($this->IsError()) {
			...
			try {
				...
				if (@openssl_pkcs7_sign($file, $signed, "file://".$this->sign_cert_file, array("file://".$this->sign_key_file, $this->sign_key_pass), NULL)) {
					...
					@unlink($file);
					@unlink($signed);
					throw new JO_Mailer_Exception($this->Lang("signing").openssl_error_string());
				}
			} catch (JO_Mailer_Exception $e) {
				$body = '';
				if ($this->exceptions) {
	...
	public function AddAttachment($path, $name = '', $encoding = 'base64', $type = 'application/octet-stream') {
		try {
			if ( !@is_file($path) ) {
				throw new JO_Mailer_Exception($this->Lang('file_access') . $path, self::STOP_CONTINUE);
			}
	...
	protected function EncodeFile($path, $encoding = 'base64') {
		try {
// 			if (!is_readable($path)) {
// 				throw new JO_Mailer_Exception($this->Lang('file_open') . $path, self::STOP_CONTINUE);
// 			}
		...
		if($this->attachmendImagesBody && preg_match_all("/(((src|background)=[\"'](.*)[\"'])|((background-image:| )\s?url\([\"'](.*)[\"']\)))/Ui", $message, $match)) {
			...
			foreach($match[4] AS $r => $img) {
				...
				if(!$img) {
					$images[] = array(
						'url' => $match[7][$r],
						'search' => '/'.preg_quote($match[0][$r],'/').'/Ui',
						'replace' => $match[6][$r]."url(\"".$cid."\")",
						'cid' => $cid,
						'filename' => $filename,
						'md5' => $md5
					);
	...
	public function set($name, $value = '') {
		try {
			if (isset($this->$name) ) {
				...
				throw new JO_Mailer_Exception($this->Lang('variable_set') . $name, self::STOP_CRITICAL);
			}
		} catch (Exception $e) {
			$this->SetError($e->getMessage());
			if ($e->getCode() == self::STOP_CRITICAL) {

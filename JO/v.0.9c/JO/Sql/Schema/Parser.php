...
class JO_Sql_Schema_Parser
{
	...
	function describe_table( $query )
	{
		...
		foreach ( $_columns as $_column ) {
			$_tidy_query .= "\t" . JO_Sql_Schema_Parser::get_column_definition( $_column ) . ",\n";
		}
		unset( $_column );
		foreach ( $_indices as $_index ) {
			$_tidy_query .= "\t" . JO_Sql_Schema_Parser::get_index_definition( $_index ) . ",\n";
		}
	...
	function _flatten_array( $array, $cut_branch = 0, $keep_child_array_keys = true )
	{
		...
		foreach ( $array as $k => $v ) {
			...
			if ( is_array( $v ) ) {
				...
				$temp += JO_Sql_Schema_Parser::_flatten_array( $v, $cut_branch, $keep_child_array_keys );
			} else {
				$temp[$k] = $v;
			}
		}
	...
	function parse( $sql )
	{
		...
		if ( is_string( $sql ) ) {
			...
			// Flatten the array
			$queries = JO_Sql_Schema_Parser::_flatten_array( $sql, 0, false );
			// Remove empty nodes
		...
		foreach ( $queries as $_query ) {
			...
			if ( !preg_match( '@^(CREATE\s+TABLE(?:\s+IF\s+NOT\s+EXISTS)?|INSERT\s+INTO|UPDATE|DELETE|TRUNCATE\s+TABLE|ALTER\s+TABLE|DROP\s+TABLE(?:\s+IF\s+EXISTS)?)\s+`?([^\s|`]+)`?@im', $_query, $_matches ) ) {
				continue;
			}
			...
			switch ( $_type ) {
				...
				case 'CREATE TABLE IF NOT EXISTS':
					$_description = JO_Sql_Schema_Parser::describe_table( $_query );
	...
	function delta( $queries, $ignore = false, $execute = true )
	{
		...
		if ( !$_queries = JO_Sql_Schema_Parser::parse( $queries ) ) {
					...
					if ( !in_array( $_new_column_data, $_existing_table_columns ) ) {
						...
						if ( !isset( $_existing_table_columns[$_new_column_name] ) ) {
							...
							$alterations['dbh_global'][$_new_table_name][] = array(
								...
								'query' => 'ALTER TABLE `' . $_new_table_name . '` ADD COLUMN ' . JO_Sql_Schema_Parser::get_column_definition( $_new_column_data ) . ';'
							);
							continue;
						}
						...
						if (
							...
							$alterations['dbh_global'][$_new_table_name][] = array(
								...
								'column' => $_new_column_name,
								'table' => $_new_table_name,
								'query' => 'ALTER TABLE `' . $_new_table_name . '` CHANGE COLUMN `' . $_new_column_name . '` ' . JO_Sql_Schema_Parser::get_column_definition( $_new_column_data ) . ';'
							);
						}
					}
				}
					...
					if ( !in_array( $_new_index_data, $_existing_table_indices ) ) {
						...
						if ( !isset( $_existing_table_indices[$_new_index_name] ) ) {
							...
							$alterations['dbh_global'][$_new_table_name][] = array(
								...
								'table' => $_new_table_name,
								'query' => 'ALTER TABLE `' . $_new_table_name . '` ADD ' . JO_Sql_Schema_Parser::get_index_definition( $_new_index_data ) . ';'
							);
							continue;
						}
...
} // END class JO_Sql_Schema_Parser
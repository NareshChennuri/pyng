...
/**
 * @see JO_Db_Adapter_Abstract
 */
require_once 'JO/Db/Adapter/Abstract.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class JO_Db_Table_Abstract
{
    ...
    /**
     * Default JO_Db_Adapter_Abstract object.
     *
     * @var JO_Db_Adapter_Abstract
     */
    protected static $_defaultDb;

    /**
     * Optional JO_Db_Table_Definition object
     *
     * @var unknown_type
     */
    ...
    /**
     * The table column names derived from JO_Db_Adapter_Abstract::describeTable().
     *
     * @var array
     */
    protected $_cols;
     ...
     * @var string
     */
    protected $_rowClass = 'JO_Db_Table_Row';
     ...
     * @var string
     */
    protected $_rowsetClass = 'JO_Db_Table_Rowset';
     ...
     * setDefinition()
     *
     * @param JO_Db_Table_Definition $definition
     * @return JO_Db_Table_Abstract
     */
    public function setDefinition(JO_Db_Table_Definition $definition)
    {
     ...
     * getDefinition()
     *
     * @return JO_Db_Table_Definition|null
     */
     ...
     * @return JO_Db_Table_Abstract Provides a fluent interface
     */
    ...
    public function getReference($tableClassname, $ruleKey = null)
    {
        ...
        if ($thisClass === 'JO_Db_Table') {
            $thisClass = $this->_definitionConfigName;
        }
        $refMap = $this->_getReferenceMapNormalized();
        if ($ruleKey !== null) {
            if (!isset($refMap[$ruleKey])) {
                require_once "JO/Db/Table/Exception.php";
                throw new JO_Db_Table_Exception("No reference rule \"$ruleKey\" from table $thisClass to table $tableClassname");
            }
            if ($refMap[$ruleKey][self::REF_TABLE_CLASS] != $tableClassname) {
                require_once "JO/Db/Table/Exception.php";
                throw new JO_Db_Table_Exception("Reference rule \"$ruleKey\" does not reference table $tableClassname");
            }
        ...
        require_once "JO/Db/Table/Exception.php";
        throw new JO_Db_Table_Exception("No reference from table $thisClass to table $tableClassname");
    }
    ...
    /**
     * Gets the default JO_Db_Adapter_Abstract for all JO_Db_Table objects.
     *
     * @return JO_Db_Adapter_Abstract or null
     */
    ...
    /**
     * Gets the JO_Db_Adapter_Abstract for this particular JO_Db_Table object.
     *
    ...
    { 
        ...
        if (is_string($db)) {
            ...
            $db = JO_Registry::get($db);
        }
        if (!$db instanceof JO_Db_Adapter_Abstract) {
            require_once 'JO/Db/Table/Exception.php';
            throw new JO_Db_Table_Exception('Argument must be of type JO_Db_Adapter_Abstract, or a Registry key where a JO_Db_Adapter_Abstract object is stored');
        }
        return $db;
    }
    ...
    /**
     * Gets the default metadata cache for information returned by JO_Db_Adapter_Abstract::describeTable().
     *
     * @return JO_Cache_Core or null
     */
    ...
    /**
     * Sets the metadata cache for information returned by JO_Db_Adapter_Abstract::describeTable().
     *
    ...
    /**
     * Gets the metadata cache for information returned by JO_Db_Adapter_Abstract::describeTable().
     *
     * @return JO_Cache_Core or null
     */
    ...
    protected static function _setupMetadataCache($metadataCache)
    {
        ...
        if (is_string($metadataCache)) {
            require_once 'JO/Registry.php';
            $metadataCache = JO_Registry::get($metadataCache);
        }
        if (!$metadataCache instanceof JO_Cache_Abstract) {
            require_once 'JO/Db/Table/Exception.php';
            throw new JO_Db_Table_Exception('Argument must be of type JO_Cache_Abstract, or a Registry key where a JO_Cache_Abstract object is stored');
        }
     ...
     * @return JO_Db_Table_Adapter_Abstract Provides a fluent interface
     */
    ...
    protected function _setupDatabaseAdapter()
    {
        if (! $this->_db) {
            ...
            if (!$this->_db instanceof JO_Db_Adapter_Abstract) {
                require_once 'JO/Db/Table/Exception.php';
                throw new JO_Db_Table_Exception('No adapter found for ' . get_class($this));
            }
        }
    }
    ...
    protected function _setupPrimaryKey()
    {
        if (!$this->_primary) {
            ...
            if (empty($this->_primary)) {
                require_once 'JO/Db/Table/Exception.php';
                throw new JO_Db_Table_Exception('A table must have a primary key, but none was found');
            }
        ...
        if (! array_intersect((array) $this->_primary, $cols) == (array) $this->_primary) {
            require_once 'JO/Db/Table/Exception.php';
            throw new JO_Db_Table_Exception("Primary key column(s) ("
         ...
         */
        if ($this->_sequence === true && $this->_db instanceof JO_Db_Adapter_Pdo_Pgsql) {
    ...
    public function info($key = null)
    {
        ...
        if (!array_key_exists($key, $info)) {
            require_once 'JO/Db/Table/Exception.php';
            throw new JO_Db_Table_Exception('There is no table information for the key "' . $key . '"');
        }
    ...
    /**
     * Returns an instance of a JO_Db_Table_Select object.
     *
     * @param bool $withFromPart Whether or not to include the from part of the select based on the table
     * @return JO_Db_Table_Select
     */
    public function select($withFromPart = self::SELECT_WITHOUT_FROM_PART)
    {
        require_once 'JO/Db/Table/Select.php';
        $select = new JO_Db_Table_Select($this);
        if ($withFromPart == self::SELECT_WITH_FROM_PART) {
            $select->from($this->info(self::NAME), JO_Db_Table_Select::SQL_WILDCARD, $this->info(self::SCHEMA));
        }
        return $select;
    }
    ...
    public function insert(array $data)
    {
        ...
        /**
         * JO_Db_Table assumes that if you have a compound primary key
    ...
    public function isIdentity($column)
    {
        ...
        if (!isset($this->_metadata[$column])) {
            /**
             * @see JO_Db_Table_Exception
             */
            require_once 'JO/Db/Table/Exception.php';

            throw new JO_Db_Table_Exception('Column "' . $column . '" not found in table.');
        }
     ...
     * @return JO_Db_Table_Rowset_Abstract Row(s) matching the criteria.
     * @throws JO_Db_Table_Exception
     */
    public function find()
    {
        ...
        if (count($args) < count($keyNames)) {
            require_once 'JO/Db/Table/Exception.php';
            throw new JO_Db_Table_Exception("Too few columns for the primary key");
        }

        if (count($args) > count($keyNames)) {
            require_once 'JO/Db/Table/Exception.php';
            throw new JO_Db_Table_Exception("Too many columns for the primary key");
        }
        ...
        foreach ($args as $keyPosition => $keyValues) {
            ...
            if ($numberTerms == 0) {
                ...
                require_once 'JO/Db/Table/Exception.php';
                throw new JO_Db_Table_Exception("Missing value(s) for the primary key");
            }
        ...
        if ($whereClause == null) {
            ...
            if (!class_exists($rowsetClass)) {
                ...
                JO_Loader::loadClass($rowsetClass);
            }
     ...
     * Fetches all rows.
     *
     * Honors the JO_Db_Adapter fetch mode.
     *
     * @param string|array|JO_Db_Table_Select $where  OPTIONAL An SQL WHERE clause or JO_Db_Table_Select object.
     * @param string|array                      $order  OPTIONAL An SQL ORDER clause.
     * @param int                               $count  OPTIONAL An SQL LIMIT count.
     * @param int                               $offset OPTIONAL An SQL LIMIT offset.
     * @return JO_Db_Table_Rowset_Abstract The row results per the JO_Db_Adapter fetch mode.
     */
    public function fetchAll($where = null, $order = null, $count = null, $offset = null)
    {
        if (!($where instanceof JO_Db_Table_Select)) {
            $select = $this->select();
    ...
    /**
     * Fetches one row in an object of type JO_Db_Table_Row_Abstract,
     * or returns null if no row matches the specified criteria.
     *
     * @param string|array|JO_Db_Table_Select $where  OPTIONAL An SQL WHERE clause or JO_Db_Table_Select object.
     * @param string|array                      $order  OPTIONAL An SQL ORDER clause.
     * @return JO_Db_Table_Row_Abstract|null The row results per the
     *     JO_Db_Adapter fetch mode, or null if no row found.
     */
     ...
     * @return JO_Db_Table_Select
     */
    protected function _where(JO_Db_Table_Select $select, $where)
    {
        $where = (array) $where;
     ...
     * @return JO_Db_Table_Select
     */
    protected function _order(JO_Db_Table_Select $select, $order)
    {
        if (!is_array($order)) {
            $order = array($order);
        }

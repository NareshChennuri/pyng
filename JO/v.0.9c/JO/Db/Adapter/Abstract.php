 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class JO_Db_Adapter_Abstract
{
     ...
     * @var integer
     */
    protected $_fetchMode = JO_Db::FETCH_ASSOC;

    /**
     * Query profiler object, of type JO_Db_Profiler
     ...
     * @var string
     */
    protected $_defaultStmtClass = 'JO_Db_Statement';
     ...
     * @var string
     */
    protected $_defaultProfilerClass = 'JO_Db_Profiler';
     ...
     * Options
     * JO_Db::CASE_NATURAL (default)
     * JO_Db::CASE_LOWER
     * JO_Db::CASE_UPPER
     *
     * @var integer
     */
    protected $_caseFolding = JO_Db::CASE_NATURAL;

    /**
     * Specifies whether the adapter automatically quotes identifiers.
     * If true, most SQL generated by JO_Db classes applies
     ...
     * Constructor.
     *
     * $config is an array of key/value pairs or an instance of JO_Config
     ...
     *
     * @param  array|JO_Config $config An array or instance of JO_Config having configuration data
    ...
    public function __construct($config)
    {
        ...
        if (!is_array($config)) {
            /*
             * Convert JO_Config argument to a plain array.
             */
            if ($config instanceof JO_Config) {
                ...
                throw new JO_Db_Adapter_Exception('Adapter parameters must be in an array or a JO_Config object');
            }
        }
        ...
        $options = array(
            JO_Db::CASE_FOLDING           => $this->_caseFolding,
            JO_Db::AUTO_QUOTE_IDENTIFIERS => $this->_autoQuoteIdentifiers
        );
        $driverOptions = array();
        ...
        if (array_key_exists(JO_Db::CASE_FOLDING, $options)) {
            $case = (int) $options[JO_Db::CASE_FOLDING];
            switch ($case) {
                case JO_Db::CASE_LOWER:
                case JO_Db::CASE_UPPER:
                case JO_Db::CASE_NATURAL:
                    $this->_caseFolding = $case;
                    break;
                default:
                    /** @see JO_Db_Adapter_Exception */
                    require_once 'JO/Db/Adapter/Exception.php';
                    throw new JO_Db_Adapter_Exception('Case must be one of the following constants: '
                        . 'JO_Db::CASE_NATURAL, JO_Db::CASE_LOWER, JO_Db::CASE_UPPER');
            }
        }

        // obtain quoting property if there is one
        if (array_key_exists(JO_Db::AUTO_QUOTE_IDENTIFIERS, $options)) {
            $this->_autoQuoteIdentifiers = (bool) $options[JO_Db::AUTO_QUOTE_IDENTIFIERS];
        }

        // obtain allow serialization property if there is one
        if (array_key_exists(JO_Db::ALLOW_SERIALIZATION, $options)) {
            $this->_allowSerialization = (bool) $options[JO_Db::ALLOW_SERIALIZATION];
        }

        // obtain auto reconnect on unserialize property if there is one
        if (array_key_exists(JO_Db::AUTO_RECONNECT_ON_UNSERIALIZE, $options)) {
            $this->_autoReconnectOnUnserialize = (bool) $options[JO_Db::AUTO_RECONNECT_ON_UNSERIALIZE];
        }

        // create a profiler object
        $profiler = false;
        if (array_key_exists(JO_Db::PROFILER, $this->_config)) {
            $profiler = $this->_config[JO_Db::PROFILER];
            unset($this->_config[JO_Db::PROFILER]);
        }
    ...
    { 
        ...
        if (! array_key_exists('dbname', $config)) {
            /** @see JO_Db_Adapter_Exception */
            require_once 'JO/Db/Adapter/Exception.php';
            throw new JO_Db_Adapter_Exception("Configuration array must have a key for 'dbname' that names the database instance");
        }
        ...
        if (! array_key_exists('password', $config)) {
             ...
             */
            require_once 'JO/Db/Adapter/Exception.php';
            throw new JO_Db_Adapter_Exception("Configuration array must have a key for 'password' for login credentials");
        }
        ...
        if (! array_key_exists('username', $config)) {
             ...
             */
            require_once 'JO/Db/Adapter/Exception.php';
            throw new JO_Db_Adapter_Exception("Configuration array must have a key for 'username' for login credentials");
        }
    }
    ...
    public function setProfiler($profiler)
    {
        ...
        if ($profilerIsObject = is_object($profiler)) {
            if ($profiler instanceof JO_Db_Profiler) {
            ...
            } else if ($profiler instanceof JO_Config) {
                ...
                throw new JO_Db_Profiler_Exception('Profiler argument must be an instance of either JO_Db_Profiler'
                    . ' or JO_Config when provided as an object');
            }
        }
        ...
        if (!$profilerInstance instanceof JO_Db_Profiler) {
            /** @see JO_Db_Profiler_Exception */
            require_once 'JO/Db/Profiler/Exception.php';
            throw new JO_Db_Profiler_Exception('Class ' . get_class($profilerInstance) . ' does not extend '
                . 'JO_Db_Profiler');
        }
     ...
     *
     * @return JO_Db_Adapter_Abstract Fluent interface
     */
    ...
    public function query($sql, $bind = array())
    {
        ...
        // is the $sql a JO_Db_Select object?
        if ($sql instanceof JO_Db_Select) {
            if (empty($bind)) {
                $bind = $sql->getBind();
            }
        ...
        // because $bind may be a JO_Db_Expr object
        if (!is_array($bind)) {
            $bind = array($bind);
        }
    ...
    {
        $this->_connect();
        $q = $this->_profiler->queryStart('begin', JO_Db_Profiler::TRANSACTION);
    ...
    {
        $this->_connect();
        $q = $this->_profiler->queryStart('commit', JO_Db_Profiler::TRANSACTION);
        $this->_commit();
        $this->_profiler->queryEnd($q);
        return $this;
    }
     ...
     *
     * @param mixed $table The table to replace data into.
     * @param array $bind Column-value pairs.
     * @return int The number of affected rows.
     */
	...
	/**
     * Inserts a table row with specified data. Ignore duplicate row
     *
     * @param mixed $table The table to insert data into.
     * @param array $bind Column-value pairs.
     * @return int The number of affected rows.
    ...
    public function update($table, array $bind, $where = '')
    {
         ...
         * except for JO_Db_Expr which is treated literally.
         */
        $set = array();
        $i = 0;
        foreach ($bind as $col => $val) {
            if ($val instanceof JO_Db_Expr) {
                ...
                if ($this->supportsParameters('positional')) {
                    ...
                    if ($this->supportsParameters('named')) {
                        ...
                        /** @see JO_Db_Adapter_Exception */
                        require_once 'JO/Db/Adapter/Exception.php';
                        throw new JO_Db_Adapter_Exception(get_class($this) ." doesn't support positional or named binding");
                    }
                }
            }
    ...
    /**
     * Convert an array, string, or JO_Db_Expr object
    ...
    /**
     * Creates and returns a new JO_Db_Select object for this adapter.
     *
     * @return JO_Db_Select
     */
    public function select()
    {
        return new JO_Db_Select($this);
    }
     ...
     *
     * @param string|JO_Db_Select $sql  An SQL SELECT statement.
    ...
    public function fetchAssoc($sql, $bind = array())
    {
        $stmt = $this->query($sql, $bind);
        $data = array();
        while ($row = $stmt->fetch(JO_Db::FETCH_ASSOC)) {
    ...
    {
        $stmt = $this->query($sql, $bind);
        $result = $stmt->fetchAll(JO_Db::FETCH_COLUMN, 0);
        return $result;
    }
    ...
    public function fetchPairs($sql, $bind = array())
    {
        $stmt = $this->query($sql, $bind);
        $data = array();
        while ($row = $stmt->fetch(JO_Db::FETCH_NUM)) {
            $data[$row[0]] = $row[1];
        }
        return $data;
    }
    ...
    public function quote($value, $type = null)
    {
        ...
        if ($value instanceof JO_Db_Select) {
            return '(' . $value->assemble() . ')';
        }

        if ($value instanceof JO_Db_Expr) {
        ...
        if ($type !== null && array_key_exists($type = strtoupper($type), $this->_numericDataTypes)) {
            ...
            switch ($this->_numericDataTypes[$type]) {
                case JO_Db::INT_TYPE: // 32-bit integer
                    $quotedValue = (string) intval($value);
                    break;
                case JO_Db::BIGINT_TYPE: // 64-bit integer
                    ...
                    break;
                case JO_Db::FLOAT_TYPE: // float or decimal
     ...
     * the adapter.
     *
     * @param string|array|JO_Db_Expr $ident The identifier.
    ...
    public function __sleep()
    {
        if ($this->_allowSerialization == false) {
            /** @see JO_Db_Adapter_Exception */
            require_once 'JO/Db/Adapter/Exception.php';
            throw new JO_Db_Adapter_Exception(get_class($this) ." is not allowed to be serialized");
        }
     ...
     *
     * @param string|JO_Db_Select $sql SQL query
     * @return JO_Db_Statement|PDOStatement
     */
